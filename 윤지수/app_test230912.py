# Anaconda Prompt 실행문 : streamlit run C:\Users\User\Desktop\app_test230911.py
...
#################################################
        ...
        with tab1:
            ...
            with tab1_1:
                ...
                st.markdown('카이제곱 검정은 두 :blue[범주형] 변수에 대한 분석 방법입니다. 예를 들면, 성별에 따른 정당 지지율 비교 문제가 이에 해당합니다.')
            ...
            with tab1_2:
                ...
                st.markdown('상관계수의 값은 항상 **+1과 -1 ** 사이의 값을 가지며, _+1은 완벽한 양의_ 선형 상관관계, _0은 선형 상관 관계 없음_,_ _-1은 완벽한 음의_ 선형 상관 관계를 의미한다.')
                st.markdown('상관계수의 절대값이 **클수록**, 즉 상관계수의 1또는 -1에 가까울 수록 **연관성이 크고**,0에 가까울 수록 연관성이 **매우 약함**을 의미한다. ')
            ...
            with tab1_4:
                ...
                st.markdown("일원배치 분산분석(One-way ANOVA)은 세 개 이상 집단 간 평균을 비교하는 통계 검정 방법입니다. 독립변수가 세 집 단 이상으로 구성된 :blue[범주형] 자료, 종속변수가 :red[연속형] 자료인 경우에 활용합니다. 집단을 나타내는 변수인 요인의 수, 즉 독립변수가 1개인 경우 일원배치 분산분석이라고 합니다.")
        ...
        with tab2:
            ...
            with tab2_2:
                ...
                st.markdown('이원배치 분산분석(TWO-Way ANOVA)은 **2 개의 독립변수에 따라 종속변수의 평균 차이를 검증**하고, 2개의 독립변수 간 **상호작용 효과를 검증**하는 방법입니다.독립변수가 2개인 경우 이원배치 분산분석이라고 합니다.')
        ...
        with tab3:
            ...
            with row3_1:
                  # 시각화를 할 두 개의 변수 선택
                column2 = st.multiselect('변수 두 개를 선택하세요', col_list,key='key1')
